...
function tsPlugin(options?: {
  ...
  return function (Parser: typeof AcornParseClass) {
    ...
    class TypeScriptParser extends Parser {
      ...
      shouldParsePrefixedArrowReturnType: any | undefined = undefined;
      ...
      tsTryParseGenericAsyncArrowFunction(
        ...
        const arrow = super.parseArrowExpression(
          res,
      ...
      parseFunctionPrefix(name: string, containsEsc: boolean) {
        if (containsEsc || this.canInsertSemicolon()) return undefined;
        if (this.ecmaVersion >= 8 && name === 'async') return { isAsync: true, isEvm: false };
        if (name === 'evm') return { isAsync: false, isEvm: true };
        return undefined;
      }

      parsePrefixedFunctionExpression(
        id: any,
        containsEsc: boolean,
        statement: number | undefined,
        allowExpressionBody: boolean,
        forInit?: boolean,
        overrideFunctionContext: boolean = false
      ) {
        const prefix = this.parseFunctionPrefix(id.name, containsEsc);
        if (!prefix || !this.eat(tt._function)) return undefined;
        if (overrideFunctionContext) this.overrideContext(tokContexts.f_expr);
      ...
      parseExprAtom(
        ...
        if (this.type === tokTypes.jsxText) {
          ...
          const prefixedFunction = this.parsePrefixedFunctionExpression(
            id,
            containsEsc,
            0,
            false,
          ...
          if (prefixedFunction) return prefixedFunction;
      ...
      shouldParsePrefixedArrow(): boolean {
        if (this.match(tt.colon)) {
          const result = this.tryParse((abort) => {
            const returnType = this.tsParseTypeOrTypePredicateAnnotation(tt.colon);
            if (this.canInsertSemicolon() || !this.match(tt.arrow)) abort();
          ...
          if (result.aborted) {
            this.shouldParsePrefixedArrowReturnType = undefined;
            return false;
          }
          if (!result.thrown) {
            if (result.error) this.setLookaheadState(result.failState);
            this.shouldParsePrefixedArrowReturnType = result.node;
      ...
      parseSubscriptPrefixedArrow(
        ...
        arrN.returnType = this.shouldParsePrefixedArrowReturnType;
        this.shouldParsePrefixedArrowReturnType = undefined;

        return this.parseArrowExpression(arrN, exprList, isAsync, forInit, isEvm);
      }
      ...
      parseSubscript(base, startPos, startLoc, noCalls, maybeAsyncArrow, optionalChained, forInit) {
        ...
        if (
          ...
          const maybePrefixedArrow = maybeAsyncArrow
            ? { isAsync: true, isEvm: false }
            : base.type === 'Identifier' &&
              base.name === 'evm' &&
              this.lastTokEnd === base.end &&
              !this.canInsertSemicolon() &&
              base.end - base.start === 3
              ? { isAsync: false, isEvm: true }
              : undefined;
          this.next();
          const oldMaybeInArrowParameters = this.maybeInArrowParameters;
          this.maybeInArrowParameters = true;
          ...
          if (maybePrefixedArrow && !optional && this.shouldParsePrefixedArrow()) {
            this.checkPatternErrors(refDestructuringErrors, false);
            this.checkYieldAwaitInDefaultParams();
            if (maybePrefixedArrow.isAsync && this.awaitIdentPos > 0)
              this.raise(
                this.awaitIdentPos,
            ...
            this.awaitPos = oldAwaitPos;
            this.awaitIdentPos = oldAwaitIdentPos;
            base = this.parseSubscriptPrefixedArrow(
              startPos,
              startLoc,
              exprList,
              forInit,
              maybePrefixedArrow.isAsync,
              maybePrefixedArrow.isEvm
            );
          } else {
            this.checkExpressionErrors(refDestructuringErrors, true);
            this.yieldPos = oldYieldPos || this.yieldPos;
